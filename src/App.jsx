...
function App() {
  ...
  return <div className="website">
    ...
    {selectedPhoto && <div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label={selectedPhoto.title} onClick={()=>setSelectedPhoto(null)}><button type="button" className="lightbox-close" onClick={()=>setSelectedPhoto(null)} aria-label="Close">×</button><div className="lightbox-content" onClick={e=>e.stopPropagation()}><img src={selectedPhoto.img} alt={selectedPhoto.title}/><div><strong>{selectedPhoto.title}</strong><span>RAJDEV PHOTOGRAPHY</span></div></div></div>}
